...
CONF_THRESHOLD = 40   # STRICT threshold (lower = stricter)
...
def match_face(face_img, label_map):
    try:
        ...
        print("DEBUG -> Label:", label, "Confidence:", confidence)
...
def verify_face():
    try:
        ...
        label_map, trained = train_model()

        if label_map is None:
            return jsonify({"error": "Training failed (no valid data)"}), 500
